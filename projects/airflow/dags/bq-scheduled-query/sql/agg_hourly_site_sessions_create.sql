-- #!{"id":"bq-hourly-sessions-create", "interval":"0 */1 * * *"}!#
create or replace table alg-hn-insights.Zz_Test_Airflow.agg_hourly_site_sessions as
(
select
main.*,
...
FROM `GA.ga_realtime_view_standard`,
    unnest(hits) h
...
(
select * from alg-hn-insights.Zz_Test_Airflow.agg_daily_site_sessions adss
where adss.utc_date >= date_add(date(current_timestamp),interval -2 day)
)